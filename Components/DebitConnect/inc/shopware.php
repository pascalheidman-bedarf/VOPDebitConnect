...
var $BIRTHDAY_TABLE = "userbilling";
var $SELECT_OFFEN = "";
var $PickwarePaymentInformation = false;
var $joinPickwarePayment = " ";
    ...
    function __construct()
	{
		...
		if(null != (DC()->settings->currentSetting->gutschriften) && DC()->settings->currentSetting->gutschriften > 0){
...
public function getBelege($limitStart,$limitEnd,$order,$filter,$fieldModes){
		...
		$dataType["order"] =  array("PK" => array(false,"rechnung.pkOrder","Key",false),
									"cRechtext" => array(true,"rechnung.cRechtext","Art",true),
									"tstamp" => array(true,"tstamp","Datum",false),
									"nRechNr" => array(true,"rechnung.nRechNr","RechnungsNr",true),
									"nRechjahr" => array(true,"rechnung.nRechJahr","RechnungsJahr",true),
									"ordernumber" => array(true,"_order.ordernumber","AuftragsNummer",true),
									"cTransaktion" => array(true,"rechnung.cTransaktion","Transaktion",false),
									"dGebucht" => array(true,"dGebucht","Verbucht am",false),
									"fZahlbetrag" => array(true,"rechnung.fZahlbetrag","Auszahlung",false),
									"id" => array(false,"rechnung.id","Auszahlung",false),
									);
		...
		if(count($filter)>0)
					{
						...
						{
							if(strlen($value)>0)	$query.=" AND $key LIKE '%".DC()->db->dbEscape($value)."%' ";
						}
					}			
					...
					//------------------------------------------
					if(count($order)>0) $query.=" ORDER BY ".$order["column"]." ".$order["direction"];
...
public function getLogBuch($limitStart,$limitEnd,$order,$filter,$fieldModes){
		$dataType["order"] =  array("PK" => array(false,"log.id","Key",false),
									"art" => array(true,"log.art","Art",true),
									"tstamp" => array(true,"log.tstamp","Datum",false),
									"logdata" => array(true,"log.logdata","Logbucheintrag",true),
									"ordernumber" => array(true,"_order.ordernumber","Auftragsnummer",true),
									"errormsg" => array(false,"log.errormsg","errormsg",false));
...
public function getShopCompanyData(){
	...
	return array("company" =>  $this->getShopwareConfValue(893,$shopID),
...
public function getZEVars($pkOrder)
{
	...
	if($output["anrede"] =="ms" ) $output["anrede"] =  "Sehr geehrte Frau" ; else $output["anrede"] =  "Sehr geehrter Herr" ;
	if($docID>0)
	{
...
public function getInvoiceDocument($docId,$dbUpload = false)
{
	try
	{
		...
		if(strlen($request["pdfDocument"])>10) return $request["pdfDocument"]; else return false;
	}catch(Exception $e)
	{
		DC()->smarty->assign("API_ERROR",$e->getMessage());
		DC()->Log("API_ERROR",$e->getMessage(),10);
    ...
    $joinPickware = strlen($this->joinPickwarePayment)>5 ? $this->joinPickwarePayment ." _order.id " : "";
			...
			$query.= " where  _order.ordernumber > 0 AND  (auftrag.trash = 0 or auftrag.trash IS NULL ) and _order.subshopID =  ".(int) DC()->settings->selectedShop;
					...
					$filterVOP = array(
					 ...
					 array("id" => 39 ,"description" => "ZE Versendet"),
					 array("id" => 55 ,"description" => "MA Bearbeitung"),
					 array("id" => 59 ,"description" => "MA Erledigt"),
					 array("id" => 95 ,"description" => "INK Bearbeitung"),
					 array("id" => 99 ,"description" => "INK Erledigt"),
					 array("id" => 100 ,"description" => "Papierkorb"),
					 array("id" => 1000 ,"description" => "Geloescht"),
					);
					...
					$output = array("order" => array(
										...
										"RechnungsNr" => array(true,"rechnung.docID","RechnungsNr",true),
										"Betrag" => array(true, "_order.invoice_amount","Betrag",false),
										"Offen" => array(true, "fWert","Offen",false),
										"Zahlung" => array(true, "zahlungseingang","Zahlung",false),
										"Datum" => array(true,"datum","Datum",false),
										"Anrede" => array(false,"billing.salutation","Anrede",false),
										"Firma" => array(true,"billing.company","Firma",true),
										"KundenNr" => array(true,"billing.customernumber","KundenNr",true),
										"Vorname" => array(true,"billing.firstname","Vorname",true),
										"Nachname" => array(true,"billing.lastname","Nachname",true),
										"Kundengruppe" => array(true,"kundengruppename","Kundengruppe",false,"arrayFilter" => $this->getCustomerGroups()),
										"Zahlart" => array(true,"zahlartname","Zahlart",false,"arrayFilter" => $this->getPaymentMethods()),
										"Zahlstatus" => array(true,"zahlstatus","Zahlstatus",false),
										"Land"  => array(false,"land.iso3","Land",false),
										"mahnstop" => array(false,"mahnstop","mahnstop",false),
										"sumGesamt" => array(false, "invoice_amount","Betrag",false),
										"sumOffen" => array(false, "fWert","Offen",false),
										"StatusVOP" => array(true, "_statusVOP","StatusVOP",false,"arrayFilter" => $filterVOP)
									)
								 , "query" => $query,"count" => $countquery);
					return $output;
}

public function getAuftragList($filter,$order,$nVOPStatus,$limit,$fieldModes = array())
{
                ...
                $joinPickware = strlen($this->joinPickwarePayment)>5 ? $this->joinPickwarePayment ." s_order.id " : "";
					...
					$output = array("order" => array(
										"PK" => array(false,"auftrag.pkOrder","Key",false),
										"Progressbarvalue"  => array(false,"status.percentvalue","progressbar",false),
										"Betrag" => array(true, "s_order.invoice_amount","Betrag",false),
										"Offen" => array($nVOPStatus == 39 || $nVOPStatus == 100 ? true : false,"fWert","Offen",false),
										"offenVOP" => array($nVOPStatus == 39 || $nVOPStatus == 100 ? false : true,"OffenVOP","Offen (VOP)",false),
										"RechnungNr" => array(true,"auftrag.cRechnungsNr","RechnungsNr",true),
										"AuftragsNr" => array(true,"s_order.ordernumber","AuftragsNr",true),
										"Datum" => array(true,"datum","Datum",false),
										"Anrede" => array(false,"auftrag.cAnrede","Anrede",false),
										"Firma" => array(true,"auftrag.cFirma","Firma",true),
										"Vorname" => array(true,"auftrag.cVorname","Vorname",true),
										"Nachname" => array(true,"auftrag.cNachname","Nachname",true),
										"Kundengruppe" => array(
										...
										"Zahlstatus" => array(true,"zahlstatus","Zahlstatus",false),
										"Land"  => array(false,"auftrag.cLand","Land",true),
										"sumGesamt" => array(false, "invoice_amount","Betrag",false),
										"sumOffen" => array(false,"OffenVOP","Offen",false)
...
public function changeOrder($pkOrder,$payment_date,$paymentstatus,$orderstatus,$comment = "",$partialpaymentstatus = null,$remove = false)
{
	try
	{
		...
		}else if($remove && DC()->settings->currentHBCI['setpaymentdate']>0){
			$params['clearedDate'] = null;
		}
		
		if($paymentstatus!=null && $paymentstatus != 'null') $params['paymentStatusId']= $paymentstatus;
		else if ($partialpaymentstatus!=null && $partialpaymentstatus != 'null') $params['paymentStatusId']= $partialpaymentstatus;
		
		if($orderstatus!=null && $orderstatus != 'null') $params['orderStatusId'] = $orderstatus;
		if(strlen($comment)>0) $params['comment'] = 	$comment;
		if(count($params)<1) return true;
			...
			else
			{
				...
				if($orderstatus>0) DC()->Log("Status","Bestellstatus gesetzt",0,$pkOrder);
				return true;
			}
	}catch(Exception $e)
		{
...
public function getDTAList($limitStart,$limitEnd,$order,$filter,$fieldModes){
	// RESET MAHNSTOP
		...
		$query = "SELECT _order.`id` , `ordernumber` , CAST(ROUND(`invoice_amount`,2) AS DECIMAL(12,2)) as invoice_amount,".$this->SELECT_OFFEN." as fWert, `ordertime` , billing.salutation, billing.company, billing.customernumber, billing.firstname, billing.lastname, billing.street, billing.zipcode, billing.city, billing.phone, land.iso3
			...
			$query.=" where ".$zahlungseingang." = '0.00' AND zahlart.id IN (".implode(",",$sepa).") AND  _order.ordernumber > 0 and IFNULL(dc_dtacreatelog.pkOrder,0) = 0  and _order.subshopID =  ".(int) DC()->settings->selectedShop;
		    $query.=" and _order.cleared in (17) ";
					...
					$output = array("order" => array(
										...
										"Nachname" => array(true,"billing.lastname","Nachname",true),
										"Kundengruppe" => array(true,"kundengruppename","Kundengruppe",false,"arrayFilter" => $this->getCustomerGroups()),
										"Zahlart" => array(true,"zahlartname","Zahlart",false),
										"Zahlstatus" => array(true,"zahlstatus","Zahlstatus",false),
										"Land"  => array(false,"land.iso3","Land",false),
										"sumGesamt" => array(false, "invoice_amount","Betrag",false),
										"sumOffen" => array(false, "fWert","Offen",false),
...
public function getOPOSList($sort,$filter,$status,$order,$nVOPStatus,$frist,$limit,$kundengruppe = array(),$minBetrag = 0,$fieldModes = array(),$cronjob = false,$withoutStates = array())
{
		// RESET MAHNSTOP
			...
			$status = count($status)>0 ? $status : array(-100);
			$b2bshop = DC()->settings->currentSetting->mahnwesenvorkasse == "on" ? true : false;
	        $paymentID = count(DC()->settings->currentPayments)> 0 ? DC()->settings->currentPayments : array(-1);
			$vorkasse = count(DC()->settings->currentVorkasse)>0 ? DC()->settings->currentVorkasse: array(-1) ;
			$sepa = count(DC()->settings->currentSEPA)>0 ? DC()->settings->currentSEPA: array(-1) ;
            $joinPickware = strlen($this->joinPickwarePayment)>5 ? $this->joinPickwarePayment ." _order.id " : "";
			$query = "SELECT _order.`id` , `ordernumber` , CAST(ROUND(`invoice_amount`,2) AS DECIMAL(12,2)) as invoice_amount,".$this->SELECT_OFFEN." as fWert, `ordertime` , billing.salutation, billing.company, billing.customernumber, billing.firstname, billing.lastname, billing.street, billing.zipcode, billing.city, billing.phone, land.iso3
			...
			$query.= " where  _order.ordernumber > 0 and  (auftrag.trash = 0 or auftrag.trash IS NULL ) and _order.subshopID =  ".(int) DC()->settings->selectedShop;
					...
					if($status)
					{
						if($nVOPStatus==0) $wherevopstatus = "<= 0" ;
						else if ($nVOPStatus == 39 ) $wherevopstatus = "<= 39 "; 
				 		else if($nVOPStatus == 90 ) $wherevopstatus = "< 55 ";
					...
					if($cronjob)  $query.= " AND IFNULL(dc_mahnstop.id,0) = 0 ";
					...
					$limitStart = (int ) 0 + (int)$limit[0];
					$limitEnd = (int)$limit[1];
					if(count($limit)>0) $query.= " LIMIT $limitStart , $limitEnd ";;
					...
					$output = array("order" => array(
										...
										"sumGesamt" => array(false, "invoice_amount","Betrag",false),
										"sumOffen" => array(false, "fWert","Offen",false),
                                         "bestellstatus" => array(true, "versandstatus.description","Bestellstatus",false),
...
public function getStandardValues(){
	$values = array();
	$values["states"] = array(9,10,11,13,14,15,16,17);
	$values["shipping"] =array("overrideInvoices" => 0, "states" => array(7));
	$values["payment"] = array(4);
	$values["vorkasse"] = array(5);
	$values["sepa"] = array(2,6);
	$values["hbci"] = array("statusbezahlt" => 12,
							"teilzahlung" => 11,
							"orderstatus" => 1,
							"setpaymentdate" => 1,
							"bestaetigung" => 0,
	...
	$values["mainsettings"] = array("shopwareapibenutzen" => 0,
									"gutschriften" => 1,
